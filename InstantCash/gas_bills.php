...
// Define database connection paracustomers
...
if ($user_balance >= $amount) {
    ...
    if ($conn->query($update_user_balance_sql) === TRUE) {
        ...
        $insertTransactionQuery = "INSERT INTO history (transaction_type, transaction_from, transaction_to, amount) 
                                  VALUES ('Gas Bill', '$transactionFrom', '$transactionTo', '$transactionAmount')";

        if ($conn->query($insertTransactionQuery) === TRUE) {
            // Insert gas bill payment record into gas_bills table
            $insertGasBillQuery = "INSERT INTO gas_bills (organization_id, organization_name, customer_no, amount, month) 
                                   VALUES ('$organization_id', '$organization_name', '$customer_no',  '$amount', '$month')";

            if ($conn->query($insertGasBillQuery) === TRUE) {
                // Update the total_received_amount for the specific organization
                $updateTotalReceivedAmountQuery = "UPDATE organization_bill SET total_received_amount = total_received_amount + '$amount' WHERE organization_id='$organization_id' AND billtype='gas_bills'";
                ...
                $error_message = "Error inserting gas bill payment record: " . $conn->error;
            }
        } else {
...
<?php
include('Front_end/front_gas_bills.php');
?>
