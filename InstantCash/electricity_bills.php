...
$organization_id = $organization_name = $amount = $month =  $meter_no = "";
$error_message = "";
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($organization_id) || empty($organization_name) || empty($amount) || empty($month) || empty($meter_no)) {
...
if ($user_balance >= $amount) {
    ...
    if ($conn->query($update_user_balance_sql) === TRUE) {
        ...
        $insertTransactionQuery = "INSERT INTO history (transaction_type, transaction_from, transaction_to, amount) 
                                  VALUES ('Electricty Bill', '$transactionFrom', '$transactionTo', '$transactionAmount')";
        ...
        if ($conn->query($insertTransactionQuery) === TRUE) {
            ...
            $insertelectricityBillQuery = "INSERT INTO electricity_bills (organization_id, organization_name, meter_no, amount, month) 
                                   VALUES ('$organization_id', '$organization_name', '$amount', '$meter_no', '$month')";

            if ($conn->query($insertelectricityBillQuery) === TRUE) {
                // Update the total_received_amount for the specific organization
                $updateTotalReceivedAmountQuery = "UPDATE organization_bill SET total_received_amount = total_received_amount + '$amount' WHERE organization_id='$organization_id' AND billtype='electricity_bills'";
